...
update_prices!(r, low_pop, high_pop, land_supply) = r .* (1 .+ (low_pop .+ high_pop) ./ land_supply)

# Simulation over time
r = copy(init_r) # Current housing prices
for t in 1:T
    # Calculate new shares for low-education workers
    ...
    low_edu_pop .= low_shares .* sum(low_edu_pop)
    high_edu_pop .= high_shares .* sum(high_edu_pop)
    ...
    # Update housing prices based on total population and fixed land supply
    update_prices!(r, low_edu_pop, high_edu_pop, land_supply)
end

# Plotting
plot(1:T, ratios[1], label="Location 1", xlabel="Time", ylabel="High-Edu to Low-Edu Ratio", title="Residential Segregation Over Time", lw=2)
